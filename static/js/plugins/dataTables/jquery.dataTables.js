...
(/** @lends <global> */function( $ ) {
	...
	function _fnAjaxDataSrc ( oSettings, json )
	{
		...
		// Compatibility with 1.9-. In orders to read from aaData, check if the
	...
	var _selector_row_indexes = function ( settings, opts )
	{
		...
		// Current page implies that orders=current and fitler=applied, since it is
	...
	var __row_selector = function ( settings, selector, opts )
	{
		return _selector_run( selector, function ( sel ) {
			...
			// Get nodes in the orders from the `rows` array (can't use `pluck`) @todo - use pluck_order
			var nodes = [];
	...
	_api_registerPlural( 'rows().nodes()', 'row().node()' , function () {
		return this.iterator( 'row', function ( settings, row ) {
			// use pluck orders on an array rather - rows gives an array, row gives it individually
	...
	$.extend( true, DataTable.ext.renderer, {
		header: {
			_: function ( settings, cell, column, idx, classes ) {
				...
				$(settings.nTable).on( 'orders.dt', function ( e, settings, sorting, columns ) {
					cell
						.removeClass(
	 ...
	 * (be it, orders, searcg or anything else). It can be used to indicate to
